 ...
 */
@Component
public class ThreadPoolManagerConcurrence implements BeanFactoryAware {
    private static Logger logger = LoggerFactory.getLogger(ThreadPoolManagerConcurrence.class);
    ...
    final RejectedExecutionHandler handler = new RejectedExecutionHandler() {
        ...
        public void rejectedExecution(Runnable r, ThreadPoolExecutor executor) {
            ...
            msgQueue.offer(((TestThread) r).getMsg());
        }
    };
        ...
        public void run() {
            if (!msgQueue.isEmpty()) {
                if (threadPool.getQueue().size() < WORK_QUEUE_SIZE) {
                    ...
                    TestThread accessDBThread = (TestThread) factory.getBean("TestThread");
    ...
    public void processOrders(String orderId) {
        if (cacheMap.get(orderId) == null) {
            ...
            TestThread accessDBThread = (TestThread) factory.getBean("dBThread");
            accessDBThread.setMsg(orderId);
            threadPool.execute(accessDBThread);
        }
    }
